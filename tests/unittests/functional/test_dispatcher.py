...
class TestDispatcher(TestCase):
    ...
    def test_local_function(self):
        ...
        retval = hyperdata.range(3).add_local().to_list()
    ...
    def test_local_function_with_schema(self):
        ...
        retval = hyperdata.new["a"]([0, 1, 2]).add_with_schema["a", "b"]()
        self.assertListEqual(retval.select("b").as_raw().to_list(), [1, 2, 3])

    def test_global_function(self):
        retval = hyperdata.range(3).add_global().to_list()
        self.assertListEqual(retval, [1, 2, 3])

    def test_global_class(self):
        retval = hyperdata.range(3).AddX(2).to_list()
        self.assertListEqual(retval, [2, 3, 4])

    def test_import_method(self):
        retval = hyperdata.range(3).ops.add_register().to_list()
        self.assertListEqual(retval, [1, 2, 3])

    def test_import_method_nested(self):
        retval = hyperdata.range(3).mock_ns.path_1.path_2.path_3().to_list()
        self.assertListEqual(retval, [1, 2, 3])
